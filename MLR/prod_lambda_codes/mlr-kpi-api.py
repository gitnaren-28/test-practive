...
USER_DOC_TABLE_NAME = get_parameter_value("MLR_USER_DOC_HISTORY")
FINDINGS_TABLE_NAME = get_parameter_value("MLR_AGENT_FINDINGS")
user_docs = dynamodb.Table(USER_DOC_TABLE_NAME)
findings_table = dynamodb.Table(FINDINGS_TABLE_NAME)

# Initialize Redshift client
redshift_data = boto3.client('redshift-data')
WORKGROUP_NAME = get_parameter_value("MLR_REDSHIFT_WORKGROUP")
DATABASE = get_parameter_value("MLR_REDSHIFT_DATABASE")
SECRET_ARN = get_parameter_value("MLR_REDSHIFT_SECRET_ARN")
...
def get_documents_by_run(run_id):
    try:
        ...
        most_matching_doc_id = None
        for doc in items:
            most_matching_doc_id = doc.get('most_matching_doc_id', None)
        ...
        return result, most_matching_doc_id
    except Exception as e:
...
def handle_get_documents_by_run(run_id):
    try:
        documents, most_matching_doc_id = get_documents_by_run(run_id)
        return response(200, {
            'documents': documents,
            'most_matching_doc_id': most_matching_doc_id,
            'run_id': run_id
        })
    except Exception as e:
