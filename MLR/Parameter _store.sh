...
aws ssm put-parameter `
  --name "MLR_CONTENT_ANALYSIS_AGENT_LAMBDA_ARN" `
  --value "arn:aws:lambda:us-east-1:969385807621:function:mlr-content-analysis-agent" `
  --type String `
  --tags Key=Name,Value=mlr

aws ssm put-parameter `
  --name "MLR_QUALITY_AGENT_LAMBDA_ARN" `
  --value "arn:aws:lambda:us-east-1:969385807621:function:mlr-quality-agent" `
  --type String `
  --tags Key=Name,Value=mlr

aws ssm put-parameter `
  --name "MLR_REFERENCE_AGENT_LAMBDA_ARN" `
  --value "arn:aws:lambda:us-east-1:969385807621:function:mlr-reference-agent" `
  --type String `
  --tags Key=Name,Value=mlr

aws ssm put-parameter `
  --name "MLR_COMPLIANCE_AGENT_LAMBDA_ARN" `
  --value "arn:aws:lambda:us-east-1:969385807621:function:mlr-compliance-agent" `
  --type String `
  --tags Key=Name,Value=mlr

aws ssm put-parameter `
  --name "MLR_CLAIMS_TABLE" `
  --value "mlr_claims_library" `
  --type String `
  --tags Key=Name,Value=mlr

aws ssm put-parameter `
  --name "MLR_BEDROCK_KB_ID" `
  --value "BE4FR77S8U" `
  --type String `
  --tags Key=Name,Value=mlr